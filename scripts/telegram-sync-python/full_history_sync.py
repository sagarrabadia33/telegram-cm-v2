...
# Smart Media Filtering (Option 2)
DOWNLOAD_VIDEOS = False  # Skip all videos (usually large)
MAX_FILE_SIZE = 5 * 1024 * 1024  # 5MB max for documents
MAX_AUDIO_SIZE = 2 * 1024 * 1024  # 2MB max for audio
DOWNLOAD_PHOTOS = True  # Photos are usually small, keep them
MEDIA_DOWNLOAD_TIMEOUT = 60  # seconds
RATE_LIMIT_DELAY = 1  # 1 second between API calls
MESSAGE_FETCH_LIMIT = 100  # Messages per API call

# Message Limits
MAX_MESSAGES_PER_CONVERSATION = 200  # Limit to latest 200 messages per chat
...
async def download_media(client: TelegramClient, message: Message) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # SMART FILTERING: Check file size before downloading
        file_size = 0
        if isinstance(media, MessageMediaDocument) and isinstance(media.document, Document):
            file_size = media.document.size

            # Skip videos if disabled (usually large and less important for text CRM)
            ...
            # Skip large audio files
            is_audio = any(
                isinstance(attr, DocumentAttributeAudio)
                for attr in media.document.attributes
            )
            if is_audio and file_size > MAX_AUDIO_SIZE:
                return None
        ...
        if isinstance(media, MessageMediaPhoto):
            if not DOWNLOAD_PHOTOS:
                return None
            # Photos are generally small (< 2MB), allow them
        ...
        try:
            ...
            log_error('media_download_timeout', Exception(f'Timeout downloading {file_name}'))
            return None
...
async def sync_contacts(client: TelegramClient, conn):
    ...
    if progress['contacts_completed']:
        print('⏭️  Contacts already synced, skipping...')
        return
    ...
    try:
        ...
        for user in users:
            ...
            try:
                ...
                # Phone formatting (E.164 standard: +country_code + number)
                if user.phone:
                    phone = f'+{user.phone}' if not user.phone.startswith('+') else user.phone
                else:
                    phone = f'telegram_{user.id}'

                # displayName priority: firstName > username > "User {id}"
                display_name = user.first_name or user.username or f'User {user.id}'

                # Parse online status from Telegram UserStatus
                is_online = False
                online_status = None
                last_seen_at = None
                ...
                if isinstance(user.status, UserStatusOnline):
                    ...
                    last_seen_at = user.status.was_online.replace(tzinfo=timezone.utc)
                ...
                metadata = Json({
                    ...
                    'accessHash': str(user.access_hash) if user.access_hash else None
                })

                # Upsert contact with ALL required fields and proper defaults
                cursor.execute("""
                    INSERT INTO "Contact" (
                        id,
                ...
                """, (
                    ...
                    display_name,
                    phone,
                    None,  # bio (don't use status string)
                    is_online,
                    online_status,
                    last_seen_at,
                    metadata
        ...
        save_progress()
        print(f'✅ Synced {progress["total_contacts"]} contacts')
...
async def sync_messages_for_conversation(client: TelegramClient, conn, conversation_id: str, dialog, chat_title: str):
    """Sync latest messages for a conversation (limited to MAX_MESSAGES_PER_CONVERSATION)."""
    print(f'  📨 Fetching latest {MAX_MESSAGES_PER_CONVERSATION} messages...')

    cursor = conn.cursor()
    offset_id = progress.get('current_message_offset', 0) or 0  # Ensure not None
    total_fetched = 0
    latest_message_time = None  # Track the actual latest message time

    try:
        async for message in client.iter_messages(dialog, limit=MAX_MESSAGES_PER_CONVERSATION, offset_id=offset_id):
            if not isinstance(message, Message):
                continue

            # Skip messages without ID (service messages)
            if not message.id:
                continue
            ...
            try:
                ...
                # Track the latest message time for updating lastMessageAt
                if latest_message_time is None or message_date > latest_message_time:
                    latest_message_time = message_date
                ...
                # Detect if message contains question
                message_body = message.message or ''
                contains_question = '?' in message_body

                # Message status (sent for outbound, received for inbound)
                message_status = 'sent' if is_outgoing else 'received'

                # Keywords array (empty for now, can be enhanced with NLP later)
                # PostgreSQL array type, not JSON
                keywords = []
                ...
                cursor.execute("""
                    ...
                    DO UPDATE SET
                        ...
                        "containsQuestion" = EXCLUDED."containsQuestion",
                ...
                """, (
                    ...
                    None,  # subject (Telegram messages don't have subjects)
                    message_body,
                    message_date,
                    message_status,
                    bool(message.media),
                ...
                if total_fetched % BATCH_SIZE == 0:
                    ...
                    save_progress()
                    print(f'    ↳ Fetched {total_fetched} messages (offset: {message.id})')
                ...
                msg_id = message.id if message.id else 'unknown'
                log_error(f'message_{msg_id}', e)
                conn.rollback()
                continue
        ...
        # Update conversation with actual latest message time (GREATEST ensures we never go backwards)
        cursor.execute("""
            UPDATE "Conversation"
        ...
        print(f'  ✅ Completed: {total_fetched} messages')
...
async def sync_conversations(client: TelegramClient, conn):
    ...
    try:
        ...
        async for dialog in client.iter_dialogs():
            try:
                ...
                if chat_title in SKIPPED_CHATS:
                    print(f'⏭️  Skipping: {chat_title}')
                    continue

                # Skip if already processed
                if str(dialog.id) in progress['processed_dialogs']:
                    print(f'⏭️  Already processed: {chat_title}')
                    continue
                ...
                print(f'\n📂 Processing: {chat_title} ({chat_type})')
                ...
                # Create conversation
                # NOTE: Do NOT set lastMessageAt from dialog.date - it's the dialog activity time, not actual message time
                # lastMessageAt will be set properly after messages are synced based on actual message timestamps
                ...
                # Create TelegramChat entry (NO updatedAt - not in schema)
                cursor.execute("""
                    INSERT INTO "TelegramChat" (id, "conversationId", "telegramChatId", "type", "title", "username", "memberCount", "isActive", "lastSyncedAt", "createdAt")
                    VALUES (
        ...
        save_progress()
        print(f'\n✅ Processed {progress["total_conversations"]} conversations')
...
async def main():
    ...
    print('🚀 Starting Telegram Full History Sync (Python/Telethon)\n')
    print('================================================')
    ...
    print(f'   Contacts: {"✅" if progress["contacts_completed"] else "⏳"}')
    print(f'   Conversations: {"✅" if progress["conversations_completed"] else "⏳"}')
