...
class ListenerStateManager:
    ...
    def record_discovery(self, discovered: int, already_known: int) -> None:
        """
        Record dialog discovery results.

        Args:
            discovered: Number of new conversations discovered
            already_known: Number of conversations already in database
        """
        cursor = self.conn.cursor()
        try:
            # Store discovery stats in connectionInfo field as JSON
            cursor.execute(f"""
                UPDATE {self.schema}."ListenerState"
                SET "connectionInfo" = jsonb_set(
                    COALESCE("connectionInfo", '{{}}'::jsonb),
                    '{{lastDiscovery}}',
                    %s::jsonb
                ),
                "lastHeartbeat" = NOW(),
    ...
    def record_error(self, error_type: str, error_message: str, context: Optional[Dict] = None) -> None:
        """
        Record an error to the database for visibility.

        Args:
            error_type: Category of error (e.g., 'dialog_discovery', 'message_sync', 'auto_create')
            error_message: The error message
            context: Optional context dict (e.g., chat_id, conversation_id)
        """
        cursor = self.conn.cursor()
        try:
            error_entry = {
            ...
            cursor.execute(f"""
                UPDATE {self.schema}."ListenerState"
                SET errors = COALESCE(errors, '[]'::jsonb) || %s::jsonb,
                    "lastHeartbeat" = NOW(),
                    "updatedAt" = NOW()
                WHERE id = 'singleton'
            """, (Json([error_entry]),))

            # Trim errors to keep only last 50
            cursor.execute(f"""
                UPDATE {self.schema}."ListenerState"
                SET errors = (
                    ...
                    FROM (
                        SELECT elem
                        FROM jsonb_array_elements(COALESCE(errors, '[]'::jsonb)) WITH ORDINALITY AS t(elem, idx)
                        ORDER BY idx DESC
                        LIMIT 50
                    ) sub
                )
    ...
    def record_conversation_created(self, chat_id: int, title: str, chat_type: str) -> None:
        """
        Record a new conversation creation for tracking.

        Args:
            chat_id: Telegram chat ID
            title: Conversation title
            chat_type: Type of chat (user/group/channel)
        """
        cursor = self.conn.cursor()
        try:
            cursor.execute(f"""
            ...
            self.conn.rollback()
            print(f"Failed to record conversation creation: {e}")
        finally:
            cursor.close()
