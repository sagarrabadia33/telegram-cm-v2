 ...
 * @param notesContext - Optional notes context for better AI understanding
...
export async function generateConversationSummary(
  ...
  const formattedMessages = messages
    ...
    .map((m) => {
      const sender = m.isOutgoing ? 'You' : (m.senderName || 'Contact');
  ...
  // Build notes context section if available
  let notesSection = '';
  if (notesContext) {
    const notesParts: string[] = [];
    if (notesContext.contactName) {
      notesParts.push(`Contact: ${notesContext.contactName}`);
    }
    if (notesContext.conversationNotes) {
      notesParts.push(`Conversation Notes: ${notesContext.conversationNotes}`);
    }
    if (notesContext.contactNotes) {
      notesParts.push(`Contact Background: ${notesContext.contactNotes}`);
    }
    if (notesParts.length > 0) {
      notesSection = `\n\nBackground Context (use this to provide more informed analysis):\n${notesParts.join('\n')}`;
    }
  }

  // Create the prompt
  const prompt = `You are analyzing a Telegram conversation titled "${conversationTitle}". Analyze the following messages and provide a structured summary.${notesSection}
